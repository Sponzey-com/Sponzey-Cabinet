...
test("knowledge graph renders the Penpot 20260713 topology with workspace data", () => {
  ...
  assert.match(html, /data-design-reference="penpot-20260713"/);
  assert.match(html, /지식 지도/);
  assert.match(html, /<strong>Cabinet 제품 지도<\/strong>/);
  assert.match(html, /<strong>로컬 저장소 설계<\/strong>/);
  ...
  assert.doesNotMatch(html, /data-action="graph-zoom-in"[^>]*disabled/);
...
test("attachments renders bounded linked document actions and opens the exact document identity", () => {
  ...
  assert.match(html, />Cabinet 제품 지도</);
  assert.match(html, />로컬 저장소 설계</);
...
test("attachments renders an explicit empty linked document state", () => {
  const html = renderToStaticMarkup(createDesktopAttachmentsElement(model, {
    ...
    detail: {
      ...
      referenceCount: 0, linkedDocumentIds: [],
    },
  }, { ...callbacks, onAssetSelect() {}, onAssetRetry() {}, onAssetImport() {}, onAssetWorkspace() {}, onAssetLink() {}, onAssetUnlink() {}, onAssetCancel() {} }));
...
test("attachments renders bounded native text preview with close action", () => {
  const html = renderToStaticMarkup(createDesktopAttachmentsElement(model, {
    ...
    detail: { assetId: "asset-1", fileName: "note.txt", mediaType: "text/plain", byteSize: 7, version: 1, previewCapability: "text", extractionStatus: "not_requested", referenceCount: 1, linkedDocumentIds: ["doc-1"] },
