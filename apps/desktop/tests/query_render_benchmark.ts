...
import {
  ...
  type QueryRenderMeasurementResult,
...
const explorationCallbacks = {
  ...
  onAssetQueryChange() {}, onAssetMediaFilterChange() {}, onAssetLoadMore() {},
...
const benchmarks = [
  ...
  {
    queryId: "search", standardFixtureCount: 10_000, boundedResultCount: 20,
    ...
    execute: async () => {
      ...
      const model = await loadDesktopDocumentNavigator({
        async getDocumentNavigator() {
          throw new Error("full-text benchmark must use searchDocuments");
        },
        async searchDocuments(query) {
          return {
            ...
            text: query.text,
            results: rows.map((index) => ({
              documentId: `doc-${index}`,
              workspaceId: query.workspaceId,
              title: `Result ${index}`,
              path: `fixture/${index}`,
              snippet: `benchmark result ${index}`,
              score: 1,
            })),
          };
        },
...
async function main(): Promise<void> {
  const results: QueryRenderMeasurementResult[] = [];
  for (const benchmark of benchmarks) {
    ...
    const result = evaluateQueryRenderMeasurement({
      queryId: benchmark.queryId,
      markerMatched,
      resultCountMatched,
      errorCount,
      p95Ms,
    ...
    console.log([
      ...
      `failure_codes=${result.failureCodes.join(",") || "none"}`,
      `p50_ms=${percentile(0.5).toFixed(6)}`,
      `p95_ms=${p95Ms.toFixed(6)}`,
      `max_ms=${percentile(1).toFixed(6)}`,
    ].join(";"));
  }
  const aggregate = aggregateQueryRenderBenchmark(results);
  console.log([
    `query_render_benchmark=${aggregate.status.toLowerCase()}`,
    `query_count=${aggregate.queryCount}`,
    `failed_queries=${aggregate.failedQueryIds.join(",") || "none"}`,
    `failure_codes=${aggregate.failureCodes.join(",") || "none"}`,
  ].join(";"));
  process.exitCode = benchmarkProcessExitCode(aggregate);
}
