...
const viewports = Object.freeze([
  { width: 1440, height: 900 }, { width: 1180, height: 800 },
  { width: 960, height: 720 }, { width: 760, height: 640 },
]);
const responsiveViewports = Object.freeze([{ width: 1440, height: 900 }]);
...
async function main() {
  ...
  requirePassed("responsive", validateResponsiveStressReport(responsive, { fingerprint: responsive.sourceFingerprint, routes, viewports: responsiveViewports }));
  ...
  const artifacts = [
    jsonArtifact("shell-geometry-phase013.json", { ...shared, marker: "phase013_shell_geometry=passed", routeCount: routes.length, viewportCount: viewports.length, maximumGeometryDeltaPx: 0 }),
    ...
    markdownArtifact("change-separation-phase013.md", "Phase 013 Tidy First Evidence", shared, [`review_slice_count=${taskCount}`, "red_green_completion_reports=verified", "feature_and_cleanup_scope_recorded=true"]),
  ...
  const evidenceByFamily = {
    ...
    LAYOUT: ["route-geometry-phase013.json", "4개 기준 viewport와 별도 200% text zoom에서 overflow가 없다."],
  ...
  const report = {
    ...
    commands: [
      ...
      { name: "four-viewport-action-geometry", state: "Passed" },
