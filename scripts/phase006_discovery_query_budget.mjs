...
export const DiscoveryQueryBudgetErrorCode = Object.freeze({
  ThresholdExceeded: "PHASE006_DISCOVERY_QUERY_BUDGET_THRESHOLD_EXCEEDED",
  EmptyMeasurement: "PHASE006_DISCOVERY_QUERY_BUDGET_EMPTY_MEASUREMENT",
});
...
export function evaluateDiscoveryQueryBudget({ thresholdMs, measurements, fixture }) {
  ...
  return {
    passed: true,
    marker: "phase006_search_graph_asset_budget=passed",
    thresholdMs,
    fixture,
    ...p95Values,
  };
...
export function renderDiscoveryQueryBudgetMarkdown(result) {
  const lines = [
    "## Phase 006 Search Graph Asset Performance Budget",
    "",
    result.marker,
    "",
...
function buildFixture({ documentCount, assetCount, graphNodeCount }) {
  ...
  for (let index = 0; index < documentCount; index += 1) {
    ...
    linksByDocument.set(documentId, { backlinks: 2, unresolved: 1 });
  }
  for (let index = 0; index < assetCount; index += 1) {
    assetsById.set(`asset-${index}`, { byteSize: index + 1 });
  }
...
function failedResult({ errorCode, thresholdMs, fixture, p95 }) {
  return {
    passed: false,
    marker: "phase006_search_graph_asset_budget=failed",
    errorCode,
    thresholdMs,
    fixture,
    ...p95,
...
async function runCli() {
  ...
  const withoutOldSection = existing.split("## Phase 006 Search Graph Asset Performance Budget")[0].trimEnd();
  await writeFile(
