...
use cabinet_domain::graph::{GraphEdge, GraphNode, GraphProjectionStatus};
    ...
    pub fn execute(
        ...
        for record in page.records() {
            ...
            for node in record.graph().nodes() {
                if nodes.len() < input.node_limit || nodes.contains_key(node.id()) {
                    nodes
            ...
            for edge in record.graph().edges() {
                if edges.len() >= input.edge_limit && !edges.contains_key(edge.id()) {
                    continue;
                }
