...
use cabinet_usecases::asset_preview::{
    AssetPreviewResult, GetAssetPreviewInput, GetAssetPreviewUsecase,
};

#[test]
fn supported_text_preview_returns_bounded_content() {
    let metadata = Catalog::new("text/plain", 7);
    let output = GetAssetPreviewUsecase::new()
        .execute(
...
fn oversized_metadata_is_rejected_before_object_read() {
    let error = GetAssetPreviewUsecase::new()
        .execute(
    ...
    fn new(media_type: &str, byte_size: u64) -> Self {
        ...
        Self(
            AssetCatalogRecord::new(
                metadata,
                1,
                AssetPreviewCapability::for_media_type(
                    &AssetMediaType::new(media_type).expect("media"),
                ),
