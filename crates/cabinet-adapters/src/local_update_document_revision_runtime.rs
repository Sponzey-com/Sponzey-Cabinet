...
use cabinet_usecases::update_document_revision::{
    UpdateDocumentRevisionError, UpdateDocumentRevisionInput, UpdateDocumentRevisionOutput,
    UpdateDocumentRevisionUsecase,
};
...
pub struct LocalUpdateDocumentRevisionRuntime {
    usecase: UpdateDocumentRevisionUsecase,
...
impl LocalUpdateDocumentRevisionRuntime {
    pub fn new(app_data_root: PathBuf, body_policy: DocumentBodyPolicy) -> Self {
        Self {
            usecase: UpdateDocumentRevisionUsecase::new(body_policy),
    ...
    pub fn execute(
        ...
        let path = self
            ...
            .map_err(|_| UpdateDocumentRevisionError::RecoveryRequired)?
            .ok_or(UpdateDocumentRevisionError::RecoveryRequired)?
            .path()
            .clone();
        let snapshot_reader = self.versions.clone();
