...
#[derive(Debug, Clone)]
pub struct LocalAssetPreviewReader {
    root: PathBuf,
}
...
impl AssetPreviewReader for LocalAssetPreviewReader {
    fn read(
        ...
        if max_bytes == 0 {
            return Err(AssetPreviewReadError::TooLarge);
        }
        let path = self.object_path(workspace, asset);
        let metadata = fs::symlink_metadata(&path).map_err(|error| {
            if error.kind() == std::io::ErrorKind::NotFound {
                AssetPreviewReadError::NotFound
            } else {
                AssetPreviewReadError::StorageUnavailable
            }
        })?;
        if !metadata.file_type().is_file() {
            return Err(AssetPreviewReadError::Corrupted);
        }
        ...
        file.take(max_bytes as u64 + 1)
            ...
            .map_err(|_| AssetPreviewReadError::StorageUnavailable)?;
        if bytes.len() > max_bytes {
            return Err(AssetPreviewReadError::TooLarge);
        }
        let actual = format!("{:x}", Sha256::digest(&bytes));
        if actual != asset.as_str() {
            return Err(AssetPreviewReadError::Corrupted);
        }
        Ok(bytes)
    }
